(() => {
  ...
  function loading(text){if(!panel)return;panel.querySelectorAll('button').forEach(b=>b.disabled=true);panel.querySelector('.ccp206-result')?.remove();const body=panel.querySelector('.ccp206-body');body.innerHTML=`<div class="ccp206-loading">${esc(text)}<div class="ccp206-spinner"><i></i><i></i><i></i></div></div>`;placePanel();}
  ...
  async function smartReply(tweet,mode){
    if(busy)return;
    busy=true;
    ...
    panel?.querySelector('.ccp206-body')?.replaceChildren();
    const inserted=await insertOnce(ai.text);
    if(!inserted.ok){setStatus('⚠️ '+inserted.error,'err');busy=false;return;}
  ...
  function showSubmenu(tweet){
    panel.innerHTML=`<div class="ccp206-head"><div class="ccp206-brand"><div class="ccp206-logo">⚡</div><div><div class="ccp206-title">Smart Reply</div><div class="ccp206-sub">Choose your writing style</div></div></div></div><div class="ccp206-body"></div>`;
    const body=panel.querySelector('.ccp206-body');
    const grid=document.createElement('div');grid.className='ccp206-grid';
    grid.appendChild(item('◉','Short','Fast & punchy',()=>smartReply(tweet,'short')));
    grid.appendChild(item('✦','Smart','Insightful & natural',()=>smartReply(tweet,'smart')));
    grid.appendChild(item('◆','Professional','Polished & credible',()=>smartReply(tweet,'professional')));
    body.appendChild(grid);
    const back=document.createElement('button');back.className='ccp206-full';back.textContent='← Back to tools';back.onclick=()=>openPanel(tweet,anchor);body.appendChild(back);placePanel();
  }
  ...
  function openPanel(tweet,button){
    ...
    panel.innerHTML=`<div class="ccp206-head"><div class="ccp206-brand"><div class="ccp206-logo">✦</div><div><div class="ccp206-title">Crypto Copilot</div><div class="ccp206-sub">AI tools for Crypto Twitter</div></div></div></div>${preview?`<div class="ccp206-context">${esc(preview)}</div>`:''}<div class="ccp206-status"><span class="dot"></span><span>Checking API…</span></div><div class="ccp206-label">AI TOOLS</div><div class="ccp206-body"><div class="ccp206-grid"></div><button class="ccp206-full" id="diag">🧪 Diagnostic&nbsp;&nbsp; <span style="color:#748097">Test API connection</span></button></div>`;
    document.body.appendChild(panel);
    const grid=panel.querySelector('.ccp206-grid');
    grid.appendChild(item('⚡','Smart Reply','Short · Smart · Pro',()=>showSubmenu(tweet)));
    grid.appendChild(item('🔁','Smart Quote','AI quote comment',()=>setStatus('Smart Quote core is ready next.')));
    grid.appendChild(item('🌍','Translate','Natural Persian',()=>translate(tweet)));
    grid.appendChild(item('✎','Rewrite','Sharper copy',()=>setStatus('Rewrite UI is ready next.')));
    grid.appendChild(item('🧵','Thread','3-post thread',()=>setStatus('Thread UI is ready next.')));
    grid.appendChild(item('📈','Trend Radar','Trend-based post',()=>setStatus('Trend Radar UI is ready next.')));
  ...
  console.log('🚀 Crypto Copilot V2.0.6 UI Core active');
})();