(() => {
  ...
  function openPanel(tweet,fab){close();currentTweet=tweet;currentFab=fab;panel=document.createElement('div');panel.className='ccp406-panel';const preview=tweetText(tweet);panel.innerHTML=`<div class="ccp406-head"><div class="ccp406-brand"><div class="ccp406-logo"><img src="data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMjgiIGhlaWdodD0iMTI4IiB2aWV3Qm94PSIwIDAgMTI4IDEyOCI+PC9zdmc+" alt=""></div><div><div class="ccp406-title">Crypto Copilot</div><div class="ccp406-sub">Natural AI tools for X</div></div></div></div>${preview?`<div class="ccp406-context">${esc(preview.slice(0,180))}${preview.length>180?'…':''}</div>`:''}<div class="ccp406-status">Connecting to Central AI…</div><div class="ccp406-body"><div style="padding:4px 3px 8px;color:#727d90;font-size:9px;font-weight:800;letter-spacing:.13em">AI TOOLS</div><div class="ccp406-grid" id="grid"></div></div>`;document.body.appendChild(panel);const g=panel.querySelector('#grid');g.appendChild(action('⚡','Smart Reply','Generate & insert automatically',()=>replyUI(tweet)));g.appendChild(action('✨','3 Reply Ideas','Analytical • Meme • Controversial',()=>replyIdeas(tweet)));g.appendChild(action('🔁','Smart Quote','Fresh quote comment',()=>quote(tweet)));g.appendChild(action('🌍','Translate','Natural Persian',()=>translate(tweet)));g.appendChild(action('✎','Rewrite','Sharper new post',()=>rewrite(tweet)));g.appendChild(action('🧵','Thread','Three connected posts',()=>thread(tweet)));g.appendChild(action('📈','Trend Radar','Use visible X trends',()=>trendRadar()));setStatus('✓ Central AI ready','ok');position();}
