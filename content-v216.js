(() => {
  if (window.__cryptoCopilotV216) return;
  window.__cryptoCopilotV216 = true;
  ...
  style.textContent = `
    .ccp216-fab{width:34px!important;height:34px!important;min-width:34px!important;margin:0 4px!important;padding:0!important;border:1px solid rgba(130,145,255,.72)!important;border-radius:11px!important;background:linear-gradient(135deg,#6478ff,#9b69ff)!important;color:#fff!important;font:800 16px/34px Arial!important;cursor:pointer!important;box-shadow:0 7px 22px rgba(90,105,255,.35)!important}
    .ccp216-panel{position:fixed;z-index:2147483647;width:370px;max-width:calc(100vw - 20px);max-height:min(650px,calc(100vh - 20px));overflow:auto;background:#090d14;color:#f4f7fb;border:1px solid rgba(255,255,255,.12);border-radius:20px;box-shadow:0 30px 90px rgba(0,0,0,.7);font:13px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;overscroll-behavior:contain}
    .ccp216-head{padding:16px;background:linear-gradient(135deg,rgba(105,121,255,.2),rgba(154,105,255,.05));border-bottom:1px solid rgba(255,255,255,.08)}
    .ccp216-brand{display:flex;align-items:center;gap:10px}.ccp216-logo{width:36px;height:36px;border-radius:11px;display:grid;place-items:center;background:linear-gradient(135deg,#6478ff,#9b69ff);font-size:17px}.ccp216-title{font-weight:850;font-size:15px}.ccp216-sub{font-size:10px;color:#8993a5;margin-top:3px}
    .ccp216-status{margin:11px 12px 4px;padding:9px 11px;border-radius:11px;background:rgba(255,255,255,.035);color:#aeb7c5;font-size:10px}.ccp216-status.ok{color:#75e2a5;background:rgba(50,210,125,.08)}.ccp216-status.err{color:#ff8796;background:rgba(255,70,95,.08)}
    .ccp216-context{margin:10px 12px;padding:11px;border:1px solid rgba(255,255,255,.07);border-radius:13px;color:#aeb7c5;font-size:11px;line-height:1.5;max-height:92px;overflow:auto}
    .ccp216-body{padding:10px}.ccp216-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.ccp216-btn,.ccp216-back{border:1px solid rgba(255,255,255,.08);border-radius:12px;background:rgba(255,255,255,.035);color:#edf0f5;cursor:pointer}.ccp216-btn{min-height:70px;padding:10px;text-align:left}.ccp216-btn:hover,.ccp216-back:hover{background:rgba(105,121,255,.1);border-color:rgba(105,121,255,.3)}.ccp216-icon{display:block;font-size:17px;margin-bottom:6px}.ccp216-btn strong{display:block;font-size:11px}.ccp216-btn span{display:block;color:#748095;font-size:9px;margin-top:3px}.ccp216-selected{border-color:rgba(115,130,255,.72)!important;background:rgba(105,121,255,.14)!important}.ccp216-generate{width:100%;margin-top:9px;padding:12px;border:0;border-radius:12px;background:linear-gradient(135deg,#6579ff,#9a69ff);color:#fff;font-weight:850;cursor:pointer}.ccp216-back{width:100%;margin-top:8px;padding:10px}.ccp216-loading{text-align:center;padding:28px;color:#aeb7c5}.ccp216-dot{display:inline-block;width:5px;height:5px;border-radius:50%;background:#7888ff;margin:12px 3px 0;animation:ccp216p 1s infinite alternate}.ccp216-dot:nth-child(2){animation-delay:.15s}.ccp216-dot:nth-child(3){animation-delay:.3s}@keyframes ccp216p{to{opacity:.2;transform:translateY(-3px)}}
    .ccp216-translation{margin:10px!important;padding:12px!important;border-radius:13px!important;background:rgba(105,121,255,.08)!important;border:1px solid rgba(105,121,255,.15)!important;color:#eef1f7!important;direction:rtl!important;line-height:1.9!important}
  `;
  ...
  function insertViaRange(editor,text){
    clearEditor(editor);
    const sel=window.getSelection();
    const range=document.createRange();
    range.selectNodeContents(editor);
  ...
  function insertViaCommand(editor,text){
    clearEditor(editor);
  ...
  async function insertIntoComposer(editor,text){
    ...
    if(before) return {ok:false,error:'Composer already contains text'};
    try {
      insertViaRange(editor,text);
      await sleep(180);
      if(editorText(editor)===normalizeText(text)) return {ok:true,actualText:editorText(editor)};
      insertViaCommand(editor,text);
      await sleep(180);
      if(editorText(editor)===normalizeText(text)) return {ok:true,actualText:editorText(editor)};
      return {ok:false,error:`Composer verification failed. Actual: ${editorText(editor)}`};
    } catch (e) {
      return {ok:false,error:e?.message||'Composer insertion failed'};
    }
  }
  ...
  function setStatus(msg,type=''){const el=panel?.querySelector('.ccp216-status');if(el){el.textContent=msg;el.className='ccp216-status '+type;position();}}
  function loading(msg){const body=panel?.querySelector('.ccp216-body');if(body){body.innerHTML=`<div class="ccp216-loading">${esc(msg)}<br><span class="ccp216-dot"></span><span class="ccp216-dot"></span><span class="ccp216-dot"></span></div>`;position();}}
  function action(icon,title,desc,fn){const b=document.createElement('button');b.type='button';b.className='ccp216-btn';b.innerHTML=`<span class="ccp216-icon">${icon}</span><strong>${esc(title)}</strong><span>${esc(desc)}</span>`;b.onclick=e=>{e.preventDefault();e.stopPropagation();fn();};return b;}
  ...
  async function translate(tweet){if(busy)return;busy=true;const source=tweetText(tweet);loading('Translating…');const r=await askAI([{role:'system',content:'Translate this Crypto Twitter text into natural Persian. Preserve names, tickers and crypto terminology. Output only the translation.'},{role:'user',content:source}],.3);if(!r.ok){setStatus(`❌ ${r.error}`,'err');busy=false;return;}let box=tweet.querySelector('.ccp216-translation');if(!box){box=document.createElement('div');box.className='ccp216-translation';tweet.appendChild(box);}box.textContent=r.text;close();}
  ...
  async function trendRadar(){const trends=visibleTrends();if(!trends.length){setStatus('⚠️ Open X Explore → Trending first','err');return;}const body=panel.querySelector('.ccp216-body');body.innerHTML=`<div style="padding:4px;color:#727d90;font-size:9px;font-weight:800;letter-spacing:.13em">VISIBLE X TRENDS</div>`+trends.map((t,i)=>`<button type="button" class="ccp216-back" data-t="${i}" style="margin-top:7px">📈 ${esc(t)}</button>`).join('');body.querySelectorAll('[data-t]').forEach(b=>b.onclick=async()=>{if(busy)return;busy=true;loading('Writing trend post…');const r=await askAI([{role:'system',content:'Write one natural Crypto Twitter post about the visible trend. Do not invent facts. Exactly one fitting emoji. Output only the post.'},{role:'user',content:`Visible trend: ${trends[Number(b.dataset.t)]}`}],.82);if(!r.ok){setStatus(`❌ ${r.error}`,'err');busy=false;return;}close();const e=await openNewPost();if(e){const out=await insertIntoComposer(e,r.text);if(!out.ok)console.warn('Crypto Copilot trend:',out.error);}busy=false;});position();}

  function replyUI(tweet){panel.innerHTML=`<div class="ccp216-head"><div class="ccp216-brand"><div class="ccp216-logo">⚡</div><div><div class="ccp216-title">Smart Reply</div><div class="ccp216-sub">Generate → insert directly into THIS Tweet</div></div></div></div><div class="ccp216-body"><div class="ccp216-grid" id="styles"></div><button class="ccp216-generate" id="gen">✨ Generate Reply</button><button class="ccp216-back" id="back">← Back</button></div>`;const styles=panel.querySelector('#styles');let selected='smart';[['◉','Short','Casual & punchy','short'],['✦','Smart','Natural & insightful','smart'],['◆','Professional','Clean & credible','professional']].forEach(([i,t,d,k])=>{const b=action(i,t,d,()=>{selected=k;styles.querySelectorAll('button').forEach(x=>x.classList.remove('ccp216-selected'));b.classList.add('ccp216-selected')});if(k==='smart')b.classList.add('ccp216-selected');styles.appendChild(b)});panel.querySelector('#gen').onclick=()=>smartReply(tweet,selected);panel.querySelector('#back').onclick=()=>openPanel(tweet,currentFab);position();}

  function openPanel(tweet,fab){close();currentTweet=tweet;currentFab=fab;panel=document.createElement('div');panel.className='ccp216-panel';const preview=tweetText(tweet);panel.innerHTML=`<div class="ccp216-head"><div class="ccp216-brand"><div class="ccp216-logo">✦</div><div><div class="ccp216-title">Crypto Copilot</div><div class="ccp216-sub">Natural AI tools for X</div></div></div></div>${preview?`<div class="ccp216-context">${esc(preview.slice(0,180))}${preview.length>180?'…':''}</div>`:''}<div class="ccp216-status">Checking API…</div><div class="ccp216-body"><div style="padding:4px 3px 8px;color:#727d90;font-size:9px;font-weight:800;letter-spacing:.13em">AI TOOLS</div><div class="ccp216-grid" id="grid"></div><button type="button" class="ccp216-back" id="test">🧪 Test API connection</button></div>`;document.body.appendChild(panel);const g=panel.querySelector('#grid');g.appendChild(action('⚡','Smart Reply','Natural • direct reply',()=>replyUI(tweet)));g.appendChild(action('🔁','Smart Quote','Fresh quote comment',()=>quote(tweet)));g.appendChild(action('🌍','Translate','Natural Persian',()=>translate(tweet)));g.appendChild(action('✎','Rewrite','Sharper new post',()=>rewrite(tweet)));g.appendChild(action('🧵','Thread','Three connected posts',()=>thread(tweet)));g.appendChild(action('📈','Trend Radar','Use visible X trends',()=>trendRadar()));panel.querySelector('#test').onclick=async()=>{if(busy)return;busy=true;loading('Testing API…');const r=await askAI([{role:'system',content:'Return exactly CCP_OK.'},{role:'user',content:'CCP_TEST'}],0);setStatus(r.ok&&r.text==='CCP_OK'?'✓ API connected':`❌ ${r.error||'API failed'}`,r.ok?'ok':'err');busy=false;};chrome.storage.local.get('openrouter_api_key',d=>setStatus(d.openrouter_api_key?'✓ API connected':'⚠️ API not configured',d.openrouter_api_key?'ok':'err'));position();}

  function attach(tweet){if(tweet.querySelector('.ccp216-fab'))return;const host=tweet.querySelector('[role="group"]')||tweet.querySelector('[data-testid="reply"]')?.parentElement;if(!host)return;const b=document.createElement('button');b.className='ccp216-fab';b.type='button';b.textContent='✦';b.title='Crypto Copilot';b.setAttribute('aria-label','Crypto Copilot');b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();openPanel(tweet,b)},{capture:true});host.appendChild(b);}
  function scan(){document.querySelectorAll('article').forEach(attach);}
  scan();new MutationObserver(scan).observe(document.body,{childList:true,subtree:true});window.addEventListener('scroll',position,true);window.addEventListener('resize',position);document.addEventListener('keydown',e=>{if(e.key==='Escape'&&panel)close();});document.addEventListener('click',e=>{if(panel&&!panel.contains(e.target)&&!e.target.closest('.ccp216-fab'))close();});
  console.log('🚀 Crypto Copilot V2.1.6 loaded');
})();
